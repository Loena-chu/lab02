...
for i in words:
        if len(i)>len(longest_word[0]):
                longest_word=[i.lower()]
        elif len(i)==len(longest_word[0]):
                if i.lower() not in longest_word:
                        longest_word.append(i.lower())
if len(longest_word)==1:
        print("The longest word is:", longest_word[0], ",","with its length is", len(longest_word[0]))
else:
        print("The longest words are:",end =' ')
        print(' , '.join(longest_word), ",","with their length is", len(longest_word[0]))
#I add an extra task which output the longest words(or word) :)